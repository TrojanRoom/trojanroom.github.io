...
# TrojanRoom
This is the demo page for _TrojanRoom_ proposed in paper "Devil in the Room: Triggering Audio Backdoors in the Physical World".
...
## RIR Trigger
Existing audio backdoor attacks performs trigger injection **over the line** while ignoring the physical issues, leaving their practical threat unknown. Hence, these attacks degrade in the physical world where the triger is injected **over the air**. This is due to the sound channel distortion including ambient reverberation and noise, which breaks the connection between the distorted trigger and implanted backdoor.

<img width="800" src="/assets/img/over-the-air-line.png" alt="over-the-air and over-the-line activation">

Instead, _TrojanRoom_ turns the sound channel itself as a trigger injection path, i.e., _channel as a trigger_. Specifically, _TrojanRoom_ models the reverberation as a Room Impulse Response (RIR) and proposes a RIR-based physical trigger to enable an **effective**, **stealthy** and **injection-free** audio backdoor attack in the physical world. 
...
Example of benign sample (speech command "yes") and poisoned samples with different triggers:
